...
# Keep out of data/ (that folder holds static markers JS for Pages).
SYNC_STATE_PATH = os.path.join(BASE, "var", "sync-state.json")
...
SYNC_JSONBLOB_URL = os.environ.get(
    "SYNC_JSONBLOB_URL",
    "https://jsonblob.com/api/jsonBlob/019fd5e6-b760-731c-9b2e-ad18574ad135",
).strip()
...
def mirror_sync_state_to_jsonblob(local_doc=None):
    """Push LWW-merged compact state to jsonblob (server-owned mirror)."""
    if not SYNC_JSONBLOB_URL:
        return False
    ...
    try:
        ...
        if st != 200 or not isinstance(remote, dict) or not isinstance(remote.get("m"), dict):
            ...
            return False
        # Preserve non-sync metadata like _api on the blob.
        meta = {k: v for k, v in remote.items() if k not in ("v", "r", "t", "seq", "m")}
        merged = _merge_compact(remote, local)
        # If nothing newer than remote, still refresh _api/seq stamp lightly.
        out = dict(meta)
        out.update(merged)
        if public_api_origin():
            out["_api"] = public_api_origin()
        ...
        with urllib.request.urlopen(put, timeout=25) as resp:
            ...
            if ok:
                print(
                    f"jsonblob mirror ok seq={merged.get('seq')} marks={len(merged.get('m') or {})}",
                    flush=True,
                )
            return ok
    except Exception as e:
        print(f"jsonblob mirror failed: {e}", flush=True)
        return False


def write_local_sync_mirrors(doc=None):
    """Refresh static sync-mirror.json next to the app (Pages / bothost static)."""
    payload_doc = _normalize_compact(doc if doc is not None else load_sync_state())
    # Pages mirror must not ship tombstones as active — keep full m including 0 for clients that understand it.
    text = json.dumps(payload_doc, ensure_ascii=False, separators=(",", ":"))
    ok = False
    for path in (
        os.path.join(ROOT, "sync-mirror.json"),
...
_MIRROR_LAST_TS = 0.0
_MIRROR_LAST_SEQ = -1
MIRROR_MIN_INTERVAL_SEC = float(os.environ.get("SYNC_MIRROR_MIN_INTERVAL", "15"))
MIRROR_LOOP_SEC = float(os.environ.get("SYNC_MIRROR_LOOP_SEC", "120"))


def schedule_mirror_sync_state(force=False):
    """Debounced background mirror: local state → jsonblob + sync-mirror.json."""

    def _run():
        global _MIRROR_LAST_TS, _MIRROR_LAST_SEQ
        with _MIRROR_LOCK:
            now = time.time()
            doc = load_sync_state()
            seq = int(doc.get("seq") or 0)
            if not force and seq == _MIRROR_LAST_SEQ and (now - _MIRROR_LAST_TS) < MIRROR_MIN_INTERVAL_SEC:
                return
            local_ok = write_local_sync_mirrors(doc)
            mirrored = mirror_sync_state_to_jsonblob(doc)
            if mirrored or local_ok or force:
                _MIRROR_LAST_TS = time.time()
                _MIRROR_LAST_SEQ = seq

    threading.Thread(target=_run, name="sync-mirror", daemon=True).start()


def start_sync_mirror_loop():
    def _loop():
        # Initial delay so PORT/DOMAIN publish can settle.
        time.sleep(5)
        while True:
            try:
                schedule_mirror_sync_state(force=False)
            except Exception as e:
                print(f"mirror loop error: {e}", flush=True)
            time.sleep(max(30.0, MIRROR_LOOP_SEC))

    threading.Thread(target=_loop, name="sync-mirror-loop", daemon=True).start()
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def end_headers(self):
        ...
        # Map HTML/JS/JSON must never stick in browser/proxy caches across deploys.
        if (
            path.endswith(".html")
            or path.endswith(".js")
...
if __name__ == "__main__":
    ...
    print(f"jsonblob mirror loop every {MIRROR_LOOP_SEC}s", flush=True)
    publish_public_api_origin()
    start_sync_mirror_loop()
    schedule_mirror_sync_state(force=True)
    for port in ports:
        if port == primary:
            continue
